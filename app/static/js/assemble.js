...
function update_assembled_prettified(code){
    ...
    code.forEach(function(code_line){
        hexed_line = Array.from(code_line).map(function(inp){return ("0"+inp.toString(16)).substr(-2).toUpperCase()}).join(' ')
...
function update_assembled_raw(code){
    ...
    code.forEach(function(code_line){
        hexed_line_raw = Array.from(code_line).map(function(inp){return "\\x"+ ("0"+inp.toString(16)).substr(-2).toUpperCase()}).join('')
